...
public class ScreenBoundaries : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        UpdateBoundariesSize();
    }

    public void UpdateBoundariesSize()
    {
        ...
        Vector2 boxColliderSize = new(ySize * mainCamera.aspect, ySize);
    ...
    public Vector2 CalculateWrappedPosition(Vector2 worldPosition)
    {
        ...
        Vector2 signWorldPosition = new(Mathf.Sign(worldPosition.x), Mathf.Sign(worldPosition.y));
        ...
        {
            return xBoundResult
                ? new Vector2(worldPosition.x * -1, worldPosition.y)
                    + new Vector2(teleportOffset * signWorldPosition.x, teleportOffset)
                : yBoundResult
                    ? new Vector2(worldPosition.x, worldPosition.y * -1)
